...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            if self.buffer :
                self.stack.append(self.buffer.pop(0))  # 버퍼에서 하나 끌어오기
                
        elif transition == "LA":
            if not self.stack[-1] == 'ROOT' and len(self.stack) > 1:
                head = self.stack.pop()
                dependency = (head, self.stack.pop())
                self.stack.append(head)
                self.dependencies.append(dependency)
                
        elif transition == "RA":
            if not self.stack[-1] == 'ROOT' and len(self.stack) > 1:
                dependency = (self.stack[-2], self.stack.pop())
                self.dependencies.append(dependency)
    ...
    while unfinished_parse:
        ...
        while mini_batch:
            ...
            for batch in mini_batch:
                if not batch.buffer and len(batch.stack) == 1:  # 버퍼가 남아있지 않고 스택에 root만 남음 -> 삭제
                    mini_batch.remove(batch)
